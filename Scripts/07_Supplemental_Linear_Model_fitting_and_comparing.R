...
# Fitting growth curves with male linear growth post-chm
...
for(i in 1:n_boots){
  ...
  boot_params_hd_lin[[i]] <- hd.temp.lin$params
  boot_params_hf_lin[[i]] <- hf.temp.lin$params
...
boot_summary<-all_boot %>%
  ...
  summarize(
    ...
    #probs based on linear form
    mean_fem_prob_hd_lin = mean(fem_prob_hd_lin, na.rm = TRUE), 
    sd_fem_prob_hd_lin = sd(fem_prob_hd_lin, na.rm = T),
    prob_hd_CI_low_lin = unname(quantile(fem_prob_hd_lin, 0.025, na.rm = TRUE)),
    prob_hd_CI_hi_lin = unname(quantile(fem_prob_hd_lin, 0.975, na.rm = TRUE)),
    
    mean_m_prob_hd_lin = mean(m_prob_hd_lin, na.rm = TRUE), 
    m_prob_hd_CI_low_lin = unname(quantile(m_prob_hd_lin, 0.025, na.rm = TRUE)),
    m_prob_hd_CI_hi_lin = unname(quantile(m_prob_hd_lin, 0.975, na.rm = TRUE)),
    
    
    mean_fem_prob_hf_lin = mean(fem_prob_hf_lin, na.rm = TRUE), 
    sd_fem_prob_hf_lin = sd(fem_prob_hf_lin, na.rm = T),
    prob_hf_CI_low_lin = unname(quantile(fem_prob_hf_lin, 0.025, na.rm = TRUE)),
    prob_hf_CI_hi_lin = unname(quantile(fem_prob_hf_lin, 0.975, na.rm = TRUE)),
    
    mean_m_prob_hf_lin = mean(m_prob_hf_lin, na.rm = TRUE), 
    m_prob_hf_CI_low_lin = unname(quantile(m_prob_hf_lin, 0.025, na.rm = TRUE)),
    m_prob_hf_CI_hi_lin = unname(quantile(m_prob_hf_lin, 0.975, na.rm = TRUE)),
...
#~~~~~linear
hd_params_df_lin <- do.call(rbind, boot_params_hd_lin)
hd_params_df_lin <- as.data.frame(hd_params_df_lin)

summary(hd_params_df_lin)


hf_params_df_lin <- do.call(rbind, boot_params_hf_lin)
hf_params_df_lin <- as.data.frame(hf_params_df_lin)

summary(hf_params_df_lin)

# get ss performances
#~~~~logistic

hd_ss_df <- data.frame(ss = do.call(rbind, ss_hd))

hf_ss_df <- data.frame(ss = do.call(rbind, ss_hf))


#~~~~linear:

hd_ss_df_lin <- data.frame(ss = do.call(rbind, ss_hd_lin))

hf_ss_df_lin <- data.frame(ss = do.call(rbind, ss_hf_lin))
...
#~~~~linear
#female
boot_summary$CI_width_HD_lin <- boot_summary$prob_hd_CI_hi_lin - boot_summary$prob_hd_CI_low_lin
boot_summary$CI_width_HF_lin <- boot_summary$prob_hf_CI_hi_lin - boot_summary$prob_hf_CI_low_lin

#male
boot_summary$CI_width_HD_male_lin <- boot_summary$m_prob_hd_CI_hi_lin - boot_summary$m_prob_hd_CI_low_lin
boot_summary$CI_width_HF_male_lin <- boot_summary$m_prob_hf_CI_hi_lin - boot_summary$m_prob_hf_CI_low_lin
